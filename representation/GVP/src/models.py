...
class CPDModel(Model):
    ...
    def extract_latent_representation(self, X, mask):
        # Method to get the result from the Encoder
        V, E, E_idx = self.features(X, mask)
        h_V = self.W_v(V)
        h_E = self.W_e(E)
        h_V = self.encoder(h_V, h_E, E_idx, mask, train=False)
        ...
        h_V_mean_pooled = tf.reduce_mean(h_V, axis=1)  # Shape: [1, 148]

        # Apply max pooling to remove the seq_num dimension
        h_V_max_pooled = tf.reduce_max(h_V, axis=1)  # Shape: [1, 148]
        return h_V_mean_pooled, h_V_max_pooled
...
node_features = (16, 100)  # 16 vector channels, 100 scalar channels
edge_features = (1, 32)    # 1 vector channel, 32 scalar channels
hidden_dim = (16, 100)     # Following the paper's setup

model = CPDModel(node_features, edge_features, hidden_dim)

checkpoint_dir = '../gvp/models/cath_pretrained'  # Replace with your correct path
...
with open('../workspace_data/pdb_json.json', 'r') as f:
# with open('../gvp/data/ts50.json', 'r') as f:
...
# protein_ids = list(protein_data.keys())
protein_ids = [protein['name'] for protein in protein_data]

representations = {}
count = 0
output_json_path = "../workspace_data/representation_gvp.json"

if os.path.exists(output_json_path):
    with open(output_json_path, 'r') as json_file:
        representations = json.load(json_file)

print(len(protein_ids))
for protein in protein_data:
    if protein['name'] in representations:
        print(f"continue with: {protein['name']}")
        continue
    # print(f"protein: {protein}")
    X = np.array(protein['coords'])  # Shape [seq_num, 4, 3]
    print(f"X: {X.shape}")
    S = np.array([ord(c) for c in protein['seq']])  # Convert sequence to integer codes. shape: seq_num
    print(f"S: {S.shape}")
    mask = np.ones_like(S)  # Assuming all residues are valid, adjust as needed. shape: seq_num
    ...
    # Reshape X to add batch dimension
    X = X[np.newaxis, ...]  # Shape [1, N, 4, 3]
    S = S[np.newaxis, ...]  # Shape [1, N]
    mask = mask[np.newaxis, ...]  # Shape [1, N]

    # Extract the latent representation from the Encoder
    h_V_mean_pooled, h_V_max_pooled = model.extract_latent_representation(X, mask)
    # print(f"Latent representation mean: {h_V_mean_pooled}")
    print(f"Latent representation mean shape: {h_V_mean_pooled.shape}") # (1, seq_num, 148) hidden_dim = (16, 100), 16是vector channels (hv)，100是scalar channels (hs)。每一个residue是3d的，所以3*16 + 100

    # print(f"Latent representation max: {h_V_max_pooled}")
    # print(f"Latent representation max shape: {h_V_max_pooled.shape}")

    representations[protein['name']] = {
                    # "protein_representation_max": h_V_max_pooled.cpu().numpy().tolist(),
                    "protein_representation_avg": h_V_mean_pooled.cpu().numpy().tolist()
                }
    del X, S, mask, h_V_mean_pooled, h_V_max_pooled

    count += 1
    if count % 10 == 0:
        with open(output_json_path, "w") as out_json_file:
            json.dump(representations, out_json_file, indent=4)
