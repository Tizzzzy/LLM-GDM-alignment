...
with open("../workspace_data/protein_representations_gearnet.json", "r") as json_file:
    protein_representations_gearnet = json.load(json_file)
...
protein_ids = list(protein_representations_gearnet.keys())
...
def create_representation_lists(protein_ids, protein_representations_gearnet, protein_representations_gemma2):
    word_representation_list = []
    graph_feature_list = []
    for protein_id in protein_ids:
        if protein_id in protein_representations_gemma2:
            graph_feature = torch.tensor(protein_representations_gearnet[protein_id]["graph_feature"]).to(device)
...
train_word_list, train_graph_list = create_representation_lists(train_ids, protein_representations_gearnet, protein_representations_gemma2)
val_word_list, val_graph_list = create_representation_lists(val_ids, protein_representations_gearnet, protein_representations_gemma2)
test_word_list, test_graph_list = create_representation_lists(test_ids, protein_representations_gearnet, protein_representations_gemma2)
print(f"length of train: {len(train_word_list)}, length of val: {len(val_word_list)}, length of test: {len(test_word_list)}")

# protein_proj_head = ProteinProjectionHead(input_dim=3072, hidden_dim=2560, output_dim=2304).to(device)
# # token_proj_head = TokenProjectionHead(input_dim=2304, hidden_dim=2304, output_dim=2304).to(device)
# token_proj_head = TokenProjectionHead(input_dim=2304, output_dim=2304).to(device)

protein_proj_head = ProteinProjectionHead(input_dim=3072, hidden_dim1=2816, hidden_dim2=2560, output_dim=2304).to(device)
# token_proj_head = TokenProjectionHead(input_dim=2304, hidden_dim1=2304, hidden_dim2=2304, output_dim=2304).to(device)
token_proj_head = TokenProjectionHead(input_dim=2304, output_dim=2304).to(device)
...
for epoch in range(num_epochs):
    ...
    if val_loss.item() < best_val_loss:
        # torch.save(protein_proj_head.state_dict(), '../workspace_data/best_protein_proj_head_gearnet_gemma2_2layer.pth')
        # torch.save(token_proj_head.state_dict(), '../workspace_data/best_token_proj_head_gearnet_gemma2_2layer.pth')
        torch.save(protein_proj_head.state_dict(), '../workspace_data/best_protein_proj_head_gearnet_gemma2_3layer.pth')
        torch.save(token_proj_head.state_dict(), '../workspace_data/best_token_proj_head_gearnet_gemma2_3layer.pth')
...
# protein_proj_head.load_state_dict(torch.load('../workspace_data/best_protein_proj_head_gearnet_gemma2_2layer.pth'))
# token_proj_head.load_state_dict(torch.load('../workspace_data/best_token_proj_head_gearnet_gemma2_2layer.pth'))
protein_proj_head.load_state_dict(torch.load('../workspace_data/best_protein_proj_head_gearnet_gemma2_3layer.pth'))
token_proj_head.load_state_dict(torch.load('../workspace_data/best_token_proj_head_gearnet_gemma2_3layer.pth'))
