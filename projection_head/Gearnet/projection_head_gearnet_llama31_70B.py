...
with open("../llama31_70b/protein_representations_llama31_70B.json", "r") as json_file:
    protein_representations_llama31 = json.load(json_file)
...
def create_representation_lists(protein_ids, protein_representations_gearnet, protein_representations_llama31):
    word_representation_list = []
    graph_feature_list = []
    for protein_id in protein_ids:
        if protein_id in protein_representations_llama31:
            graph_feature = torch.tensor(protein_representations_gearnet[protein_id]["graph_feature"]).to(device)
            word_representation = torch.tensor(protein_representations_llama31[protein_id]["word_representation"]).to(device) # (2304, )
...
train_word_list, train_graph_list = create_representation_lists(train_ids, protein_representations_gearnet, protein_representations_llama31)
val_word_list, val_graph_list = create_representation_lists(val_ids, protein_representations_gearnet, protein_representations_llama31)
test_word_list, test_graph_list = create_representation_lists(test_ids, protein_representations_gearnet, protein_representations_llama31)
print(f"length of train: {len(train_word_list)}, length of val: {len(val_word_list)}, length of test: {len(test_word_list)}")

protein_proj_head = ProteinProjectionHead(input_dim=3072, output_dim=8192).to(device)
token_proj_head = TokenProjectionHead(input_dim=8192, output_dim=8192).to(device)
...
for epoch in range(num_epochs):
    ...
    if val_loss.item() < best_val_loss:
        torch.save(protein_proj_head.state_dict(), '../workspace_data/best_protein_proj_head_gearnet_llama31_70B.pth')
        torch.save(token_proj_head.state_dict(), '../workspace_data/best_token_proj_head_gearnet_llama31_70B.pth')
...
protein_proj_head.load_state_dict(torch.load('../workspace_data/best_protein_proj_head_gearnet_llama31_70B.pth'))
token_proj_head.load_state_dict(torch.load('../workspace_data/best_token_proj_head_gearnet_llama31_70B.pth'))
