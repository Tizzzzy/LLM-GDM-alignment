...
train_word_list, train_graph_list = create_representation_lists(train_ids, protein_representations_gvp, protein_representations_gemma2)
val_word_list, val_graph_list = create_representation_lists(val_ids, protein_representations_gvp, protein_representations_gemma2)
test_word_list, test_graph_list = create_representation_lists(test_ids, protein_representations_gvp, protein_representations_gemma2)
...
# protein_proj_head = ProteinProjectionHead(input_dim=148, hidden_dim=1024, output_dim=2304).to(device)
protein_proj_head = ProteinProjectionHead(input_dim=148, hidden_dim1=512, hidden_dim2=1024, output_dim=2304).to(device)
...
for epoch in range(num_epochs):
    ...
    if val_loss.item() < best_val_loss:
        # torch.save(protein_proj_head.state_dict(), '../workspace_data/best_protein_proj_head_gvp_gemma2_2layer.pth')
        # torch.save(token_proj_head.state_dict(), '../workspace_data/best_token_proj_head_gvp_gemma2_2layer.pth')
        torch.save(protein_proj_head.state_dict(), '../workspace_data/best_protein_proj_head_gvp_gemma2_3layer.pth')
        torch.save(token_proj_head.state_dict(), '../workspace_data/best_token_proj_head_gvp_gemma2_3layer.pth')
...
protein_proj_head.load_state_dict(torch.load('../workspace_data/best_protein_proj_head_gvp_gemma2_3layer.pth'))
token_proj_head.load_state_dict(torch.load('../workspace_data/best_token_proj_head_gvp_gemma2_3layer.pth'))
